...
def get_video_output_path(filename="bohemian.mp4"):
    out_dir = os.path.join(os.getcwd(), "output")
    os.makedirs(out_dir, exist_ok=True)
    return os.path.join(out_dir, filename)

def get_frame_output_path(i, prefix="frame"):
    out_dir = os.path.join(os.getcwd(), "output")
    os.makedirs(out_dir, exist_ok=True)
    return os.path.join(out_dir, f"{prefix}_{i:03}.png")
...
def plot_eigenvalues(matrix, eigenvalues_list, fig, ax, frames,i, desired_shape=(7680, 4320)):
    ...
    ax.scatter(all_eigenvalues.real, all_eigenvalues.imag, marker='.', s=0.69, color='blue', alpha=0.7)
    ax.set_xlim([-1, 1])  
    ax.set_ylim([-1, 1])  
    ax.set_xticks([])  
    ax.set_yticks([]) 
    ...
    cv2.imwrite(get_frame_output_path(i), cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
    frames.append(image)
...
def make_frames(size, matrix_0, matrix_1, num_interpolations, num_samples_per_interpolation):
    ...
    fig = plt.figure(figsize=(desired_shape[0] / 100, desired_shape[1] / 100), dpi=50)
...
def make_video(pathOut,fps, size, matrix_array, num_interpolations, num_samples_per_interpolation):
    ...
    out = cv2.VideoWriter(pathOut, cv2.VideoWriter_fourcc(*'mp4v'), fps, size) # or try: *'H264'
...
def main():
    ...
    profiler.enable()
    make_video(pathOut,fps, size, matrix_array, 5, 1000_000)
    profiler.disable()
